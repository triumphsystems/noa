...
describe('Rate Limiting & Throttling Defense Suite', () => {
  ...
  it('should compute consistent window slots for rate limiting', () => {
    ...
    assert.equal(
      slot1,
      slot2,
      'Timestamps in the same minute must map to the same window slot'
    );
    assert.notEqual(
      slot1,
      slot3,
      'Timestamps in different minutes must map to different window slots'
    );
    assert.equal(
      slot3 - slot1,
      60,
      'Window slot step must equal windowSeconds'
    );
  });
  ...
  it('should prioritize custom user/doctor ID over IP address', () => {
    function resolveIdentifier(customId, cookieUserId, clientIp) {
      if (customId && customId.trim().length > 0)
        return `custom:${customId.trim()}`;
      if (cookieUserId && cookieUserId.trim().length > 0)
        return `auth:${cookieUserId.trim()}`;
    ...
    assert.equal(resolveIdentifier(null, null, null), 'ip:127.0.0.1');
  });
