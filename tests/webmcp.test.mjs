...
describe('WebMCP Core Protocol & Registry Suite', async () => {
  ...
  it('should match RFC 6570 resource URI patterns accurately', () => {
    const templates = [
      ...
      {
        template: 'patient://{patientId}/history',
        regex: /^patient:\/\/([^/]+)\/history$/,
        keys: ['patientId'],
      },
      {
        template: 'doctor://{doctorId}',
        regex: /^doctor:\/\/([^/]+)$/,
        keys: ['doctorId'],
      },
      {
        template: 'doctor://{doctorId}/dashboard',
      ...
      {
        template: 'session://{sessionId}/transcript',
        regex: /^session:\/\/([^/]+)\/transcript$/,
        keys: ['sessionId'],
      },
      {
        template: 'soap://{sessionId}',
        regex: /^soap:\/\/([^/]+)$/,
        keys: ['sessionId'],
      },
      {
        template: 'intake://{patientId}',
        regex: /^intake:\/\/([^/]+)$/,
        keys: ['patientId'],
      },
      {
        template: 'stats://doctor/{doctorId}',
        regex: /^stats:\/\/doctor\/([^/]+)$/,
        keys: ['doctorId'],
      },
    ];
  ...
  it('should generate clinical prompt messages adhering to MCP prompt schema', () => {
    const promptDefinitions = [
      {
        ...
        arguments: [
          {
            name: 'transcript',
            description: 'Audio transcript text',
            required: true,
          },
          {
            name: 'patientContext',
    ...
    const messages = prompt.generator({
      transcript: 'Doctor: Hello. Patient: Sore throat.',
    });
...
describe('WebMCP JSON-RPC 2.0 Dispatcher Suite', async () => {
          ...
          case 'tools/call':
            if (request.params?.name === 'generate_soap_note') {
              return {
                ...
                result: {
                  content: [
                    { type: 'text', text: 'SOAP note generated successfully' },
                  ],
                },
              };
            }
  ...
  it('should handle "tools/call" method and execute tool', async () => {
    ...
    assert.ok(
      res.result.content[0].text.includes('SOAP note generated successfully')
    );
  });
  ...
  it('should process batch JSON-RPC request arrays', async () => {
    ...
    const responses = await Promise.all(
      batch.map((req) => dispatcher.dispatch(req))
    );
...
describe('Browser-Native document.modelContext Runtime Suite', async () => {
  ...
  it('should support dynamic tool registration and execution in browser context', async () => {
    ...
    // Execute tool
    const result = await runtime.executeTool('search_products', {
      query: 'antibiotics',
    });
...
describe('Clinical AI Provider Engine & Zero-Mock Safety Suite', async () => {
  it('should default to Bedrock provider and resolve correct model tiers', () => {
    function resolveProvider(envVal) {
      return (envVal || 'bedrock').toLowerCase() === 'local'
        ? 'local'
        : 'bedrock';
    }
    ...
    assert.equal(
      resolveModel('bedrock', 'reasoning'),
  ...
  it('should fail-fast with ClinicalAIUnavailableError and never return fake mock data', async () => {
    class ClinicalAIUnavailableError extends Error {
      constructor(provider, model, originalError, remediation) {
        super(
          `Clinical AI service unavailable via ${provider} [${model}]. ${remediation}`
        );
