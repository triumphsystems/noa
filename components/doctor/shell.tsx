...
interface DoctorLayoutShellProps {
  doctor: Doctor | null;
...
export function DoctorLayoutShell({ doctor, children }: DoctorLayoutShellProps) {
  ...
  // Synchronize doctor into Zustand store for child client components
  useEffect(() => {
    if (doctor) {
      useDoctorStore.getState().setDoctorId(doctor.id);
      useDoctorStore.setState({ doctor, isLoading: false, error: null });
    }
  }, [doctor]);
