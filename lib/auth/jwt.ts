...
async function verifyRS256(
  ...
  try {
    ...
    return crypto.subtle.verify('RSASSA-PKCS1-v1_5', key, sig as unknown as BufferSource, data as unknown as BufferSource)
  } catch {
    return false
  }
}
...
async function verifyTokenWithSignature(token: string): Promise<VerifiedAuthPayload> {
  ...
  if (!userPoolId) {
    ...
    if (typeof payload.exp === 'number' && payload.exp < now) return { isValid: false }
...
function buildPayload(payload: Record<string, unknown>): VerifiedAuthPayload {
  ...
  if (customType === 'doctor' || customType === 'patient' || customType === 'admin') {
    userType = customType
 ...
 */
export async function getAuthenticatedUser(request: NextRequest): Promise<VerifiedAuthPayload> {
  ...
  if (sessionMeta) {
    try {
      ...
      if (!verified.userType && parsed.userType) verified.userType = parsed.userType
 ...
 */
export function getAuthenticatedUserSync(request: NextRequest): VerifiedAuthPayload {
  const idToken = request.cookies.get(AUTH_COOKIE_NAMES.ID_TOKEN)?.value
