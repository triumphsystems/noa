...
export function handleAuthExpiration() {
  ...
  const loginUrl = `/auth/login?expired=1&type=${storedUserType}${
    currentPath && !currentPath.startsWith('/auth') ? `&from=${encodeURIComponent(currentPath)}` : ''
  }`
...
export async function http<T = any>(input: RequestInfo | URL, init?: RequestInit): Promise<T> {
  ...
  if (response.status === 401) {
    const isRefreshEndpoint = typeof input === 'string' && input.includes('/api/auth/refresh')
...
http.post = <T = any>(url: string, body?: any, init?: Omit<RequestInit, 'method' | 'body'>) =>
  http<T>(url, {
    ...init,
    method: 'POST',
    headers: {
...
http.put = <T = any>(url: string, body?: any, init?: Omit<RequestInit, 'method' | 'body'>) =>
  http<T>(url, {
    ...init,
    method: 'PUT',
    headers: {
