    ...
    if (error?.name === 'ResourceNotFoundException' || error?.name === 'GroupNotFoundException') {
      await cognito.send(
        new CreateGroupCommand({
          UserPoolId: userPoolId,
          GroupName: 'Admins',
          Description: 'Superadministrators and clinical review officers with full administrative access',
          Precedence: 0,
        })
      )
      return true
...
async function main() {
  ...
  console.log(`  ${symbols.bullet} AWS Region:        ${c.cyan}${region}${c.reset}`)
  console.log(`  ${symbols.bullet} Cognito Pool ID:   ${c.cyan}${userPoolId}${c.reset}`)
  console.log(`  ${symbols.bullet} DynamoDB Table:    ${c.cyan}${tableName}${c.reset}`)
  console.log()
  ...
  while (!email || !email.includes('@')) {
    email = await promptQuestion(rl, 'Superadmin Email', 'e.g. medical.director@noa.health')
  ...
  while (password.length < 6) {
    ...
    if (password.length < 6) {
      console.log(`   ${c.rose}Password must be at least 6 characters long.${c.reset}`)
    }
  }
  ...
  try {
    ...
    process.stdout.write(`   ${c.slate}[1/5] Checking Cognito "Admins" group...${c.reset}`)
    const created = await ensureAdminGroup()
    process.stdout.write(`\r   ${symbols.check} [1/5] Cognito "Admins" group verified ${created ? '(created new)' : '(existing)'}\n`)

    // Step 2: Create User
    process.stdout.write(`   ${c.slate}[2/5] Creating user identity in Cognito...${c.reset}`)
    let userSub = ''
    try {
      const createCommand = new AdminCreateUserCommand({
        ...
        UserAttributes: [
          ...
          { Name: 'family_name', Value: name.split(' ').slice(1).join(' ') || 'Admin' },
        ],
        MessageAction: 'SUPPRESS',
      })

      const createRes = await cognito.send(createCommand)
      userSub = createRes.User?.Attributes?.find(a => a.Name === 'sub')?.Value || ''
      process.stdout.write(`\r   ${symbols.check} [2/5] User identity registered in Cognito\n`)
    } catch (err) {
      if (err?.name === 'UsernameExistsException') {
        process.stdout.write(`\r   ${symbols.check} [2/5] User already exists in Cognito (updating profile)\n`)
      } else {
        throw err
      }
    }
    ...
    process.stdout.write(`   ${c.slate}[3/5] Setting permanent password...${c.reset}`)
    await cognito.send(
    ...
    process.stdout.write(`\r   ${symbols.check} [3/5] Permanent password configured\n`)

    // Step 4: Add to Admins Group
    process.stdout.write(`   ${c.slate}[4/5] Assigning user to "Admins" group...${c.reset}`)
    await cognito.send(
    ...
    process.stdout.write(`\r   ${symbols.check} [4/5] Administrator group membership granted\n`)

    // Step 5: DynamoDB Record
    process.stdout.write(`   ${c.slate}[5/5] Registering admin record in DynamoDB...${c.reset}`)
    const adminId = userSub || `admin-${Buffer.from(email).toString('hex').slice(0, 12)}`
    ...
    process.stdout.write(`\r   ${symbols.check} [5/5] DynamoDB admin profile stored (${adminId})\n`)
